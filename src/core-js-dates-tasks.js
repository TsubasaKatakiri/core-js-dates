...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const dateDiff = Date.parse(dateEnd) - Date.parse(dateStart);
...
function getCountWeekendsInMonth(month, year) {
  ...
  let dayCount = 0;
  do {
    if (startDate.getDay() === 6 || startDate.getDay() === 0) dayCount += 1;
...
function getWeekNumberByDate(date) {
  ...
  const dateWeekDay = firstDate.getDay();
  if (dateWeekDay > 1) {
    firstDate.setDate(firstDate.getDate() + 7 - (dateWeekDay - 1));
  } else if (dateWeekDay < 1) {
    firstDate.setDate(firstDate.getDate() + 1);
  }
  const endDate = new Date(firstDate);
  endDate.setDate(endDate.getDate() + 6);
  let week = firstDate.getDate() < 5 ? 0 : 1;
  while (!(date >= firstDate && date <= endDate)) {
    week += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    if (counterWork < countWorkDays) {
      counterOff = 0;
      counterWork += 1;
      result.push(
        startDate.toISOString().split('T')[0].split('-').reverse().join('-')
      );
    } else if (counterOff < countOffDays) {
      counterOff += 1;
    } else {
      counterOff = 0;
      counterWork = 1;
